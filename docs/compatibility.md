...
The CI workflow now runs this installer and matting smoke on Windows, macOS and Ubuntu with Node 22/24, forcing the project-local FFmpeg fallback. Current execution results are available in [Portable toolchain runs](https://github.com/erduo1998-cell/agent-motion/actions/workflows/ci.yml). The earlier September 19 table above describes the earlier toolchain, not evidence for this newly added installation path. Synthetic inference tests validate plumbing; human edges and full-film quality still require inspection.
