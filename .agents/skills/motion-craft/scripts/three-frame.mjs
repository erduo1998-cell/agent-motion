...
function adapterVector(value, name) {
  const a = Array.isArray(value)
    ? value
    : value && Number.isFinite(value.x) && Number.isFinite(value.y) && Number.isFinite(value.z)
      ? [value.x, value.y, value.z]
      : null;
  if (!a || a.length !== 3) throw new TypeError(`${name} must be [x, y, z] or a vector-like object`);
  a.forEach(component => finite(component, name));
  return [...a];
}

function normalized(value, name) {
  const v = adapterVector(value, name);
  return mul(v, 1 / positive(Math.hypot(...v), `${name} length`));
}

function basisFromView(viewFrom, worldUp) {
  const backward = normalized(viewFrom, 'viewFrom');
  const forward = mul(backward, -1);
  const upHint = normalized(worldUp, 'worldUp');
  let up = sub(upHint, mul(forward, dot(upHint, forward)));

  if (Math.hypot(...up) < 1e-10) {
    let right = sub([1, 0, 0], mul(forward, forward[0]));
    if (Math.hypot(...right) < 1e-10) right = sub([0, 0, 1], mul(forward, forward[2]));
    right = normalized(right, 'fallback camera right');
    up = normalized(cross(right, forward), 'fallback camera up');
    return { right, up, forward };
  }

  up = normalized(up, 'orthogonal worldUp');
  const right = normalized(cross(forward, up), 'camera right');
  up = normalized(cross(right, forward), 'camera up');
  return { right, up, forward };
}

function assertSymmetricPerspectiveCamera(camera) {
  if (!camera?.isPerspectiveCamera) throw new TypeError('camera must be a THREE.PerspectiveCamera');
  if (camera.zoom !== 1 || camera.filmOffset !== 0 || camera.view?.enabled) {
    throw new RangeError('camera must use a symmetric perspective projection (zoom=1, filmOffset=0, no view offset)');
  }
}

function objectWorldBoxCorners(THREE, object, index) {
  if (!object?.isObject3D) throw new TypeError(`objects[${index}] must be a THREE.Object3D`);
  object.updateWorldMatrix(true, true);
  const box = new THREE.Box3().setFromObject(object, true);
  if (box.isEmpty()) throw new RangeError(`objects[${index}] has empty world bounds`);
  const corners = [];
...
/** Compute a world-space camera pose from current world Box3 corners. */
export function frameObjectPose({
  THREE,
  camera,
  objects,
  rect = { x: 0, y: 0, w: 1, h: 1 },
  ...
  if (!THREE?.Box3) throw new TypeError('THREE namespace with Box3 is required');
  assertSymmetricPerspectiveCamera(camera);
  if (!Array.isArray(objects) || objects.length === 0) throw new RangeError('objects must be a nonempty Object3D array');
  const points = objects.flatMap((object, index) => objectWorldBoxCorners(THREE, object, index));
  return solvePerspectiveFraming({
    points,
    basis: basisFromView(viewFrom, worldUp),
...
/** Apply a solved world pose. The camera must live under an untransformed root. */
export function applyFramePose(camera, pose) {
  assertSymmetricPerspectiveCamera(camera);
  if (!pose || !Array.isArray(pose.position) || !Array.isArray(pose.target) || !Array.isArray(pose.up)) {
    throw new TypeError('pose must contain position, target, and up arrays');
  }
  camera.position.fromArray(pose.position);
  camera.up.fromArray(pose.up);
  camera.lookAt(...pose.target);
  if (Number.isFinite(pose.depthRange?.far) && pose.depthRange.far >= camera.far) {
    camera.far = pose.depthRange.far + Math.max(1, pose.depthRange.far) * 1e-6;
  }
...
/** Compute and immediately apply a pose. */
export function fitObjectsToFrame(args) {
  const pose = frameObjectPose(args);
  return applyFramePose(args.camera, pose);
}
